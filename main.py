...
vgg = VGG('sgd' , False , True,   model_name, 'gap'  , 'cifar10' , 60 , resize=(350,350) , num_epoch=100)
"""
resnet_v1=RESNET_V1(args.opt , args.use_bn , args.l2_weight_decay, args.logit_type , args.loss_type ,args.datatype ,\
                    args.batch_size, args.cropped_size,args.num_epoch ,args.init_lr, args.lr_decay_step, \
                    args.model_name ,args.aug_list)

recorder = Recorder(folder_name=args.model_name)
trainer = Trainer(recorder ,train_iter = 100 )
tester=Tester(recorder)

test_imgs, test_labs ,fnames =resnet_v1.dataprovider.reconstruct_tfrecord_rawdata(resnet_v1.dataprovider.test_tfrecord_path , (350,350))
#test_labs=utils.cls2onehot(test_labs, resnet_v1.n_classes)
val_imgs, val_labs ,fnames =resnet_v1.dataprovider.reconstruct_tfrecord_rawdata(resnet_v1.dataprovider.val_tfrecord_path , (350,350))
#val_labs=utils.cls2onehot(val_labs, resnet_v1.n_classes)

test_labs=np.reshape(test_labs , (-1 , 1))
val_labs=np.reshape(val_labs, (-1 , 1))
if np.max(test_imgs) > 1 :
    test_imgs = test_imgs / 255.
if np.max(val_imgs) > 1:
    val_imgs = val_imgs / 255.
