...
class Tester(DNN):
    ...
    def validate(self , fetches , imgs , labs , batch_size):
        """
        #### Validate ###
        ...
        if share !=0:
            ...
            mean_loss, pred_all = self.sess.run(fetches=fetches, feed_dict=test_feedDict)

        mean_loss=np.mean(mean_loss)
        mean_acc = self.get_acc(labs,  pred_all)
        self.recorder.write_acc_loss(prefix='Test' , loss=mean_loss , acc= mean_acc)
        return mean_acc ,mean_loss , preds